...
The smart thing is that you don't have to define the image names in the workflow file since it gets them from the `compose.prod.yaml` file.

## The workflow file

The workflow file does the following:

1. Checks out the repository. This way it has access to the files in the repository, including the `compose.prod.yaml` file.
...
```yaml
    - name: Build and Push Docker Images
      ...
      run: |
        docker buildx bake -f compose.prod.yaml --push
```
...
2. Change the two image names in `compose.prod.yaml` to your own.
